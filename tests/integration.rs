...
use neural_sim::synapse::plasticity::{StdpRule, ConsolidationRule};
...
fn test_brain_builder_with_simulation() {
    ...
    let network = BrainBuilder::new()
        ...
        .add_region("Input", 100, 0.80, NeuronModelParams::Izhikevich {
            a: 0.02, b: 0.2, c: -65.0, d: 8.0,
        })
        .mark_input("Input")
        .add_cortical_column("V1", 300)
        .add_region("Output", 100, 0.80, NeuronModelParams::Izhikevich {
            a: 0.02, b: 0.2, c: -65.0, d: 8.0,
        })
        .mark_output("Output")
    ...
    assert!(stats.sim_time_ms >= 99.0, "sim_time_ms={} should be near 100ms", stats.sim_time_ms);
    ...
    assert!(stats.weight_updates > 0, "STDP should produce weight updates");
    ...
    let output_rate = {
        let net = engine.network.read();
        net.region_rates().iter()
            .find(|(name, _)| name == "Output")
            .map(|(_, rate)| *rate)
            .unwrap_or(0.0)
    };
    assert!(output_rate >= 0.0, "Output region should report firing rate");
}
...
fn test_brain_builder_conductance_simulation() {
    ...
    let network = BrainBuilder::new()
        .add_region("A", 100, 0.80, NeuronModelParams::Izhikevich {
            a: 0.02, b: 0.2, c: -65.0, d: 8.0,
        })
        .build();
    ...
    assert!(stats.sim_time_ms >= 49.0, "sim_time_ms={} should be near 50ms", stats.sim_time_ms);
}
...
fn test_brain_builder_stdp_weight_change() {
    ...
    let mut network = BrainBuilder::new()
        .add_region("Test", 150, 0.80, NeuronModelParams::Izhikevich {
